...
class SpellChecker:
    ...
    def _run_checks(self, inputs, outputs):
        while True:
            args = inputs.get()
            if args is None:
                outputs.put(None)
            ...
            while True:
                ...
                if output is None:
                    ...
                    if isinstance(output, Exception):
                        err = self.get_error(output)
                        yield Results([], source.context, source.category, err)
                    else:
                        yield Results(
                            [w for w in sorted(set(output.replace('\r', '').split('\n'))) if w],
                            source.context,
                            source.category
                        )
